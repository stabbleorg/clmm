...
use crate::states::{DynamicTick, PersonalPositionState, Tick, TickUpdate};
...
impl TickArrayRentTransfer {
    fn execute<'info>(
        &self,
        position: &Account<'info, PersonalPositionState>,
...
pub fn calculate_modify_tick_array(
    ...
    if position_liquidity_before == 0 && position_liquidity_after != 0 {
    ...
    transfer(
        CpiContext::new(
            ...
            Transfer {
                ...
                to: position_account.to_account_info(),
            },
        ),
        rent_amount,
    )?;
